...
void *client_operation(void *arg)
{
    ...
    while (connected)
    {
        ...
        json incomingMessage = convert_to_json(requestData);
        cout << "Client with fd " << client_fd << " chose " << incomingMessage["type"] << endl;
        print_server_state();
        // if (requestType == static_cast<int>(RequestType::Login))
        // {
        //     cout << "Received login request" << endl;
        //     handleLoginRequest(json_data["data"], clientSocket);
        // }
        int option = incomingMessage["type"];

        switch (option)
        {
        case static_cast<int>(RequestType::Login):
            cout << "Received login request" << endl;
            handleLoginRequest(incomingMessage["data"], client_fd);
            break;
        case static_cast<int>(RequestType::MatchMaking): // Random matchmaking:
